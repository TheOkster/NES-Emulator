      ...
      for cycle in range(341):
         ...
         for _ in range(18):
            ...
            if dut.pixel_valid.value:
               assert 0 <= x < 256 and 0 <= y < 240, f"x is {x} and y is {y}"
               assert x == dut.pixel_x.value and y == dut.pixel_y.value
               pixelValidAtSomePoint = True
         if 0 <= x < 256 and 0 <= y < 240:
            assert pixelValidAtSomePoint, f"pixel not valid at some point for {(x, y)}"
