...
# async def test_uart_reciever(dut):
#     recv_byte = 0
#     while True:
#         await FallingEdge(dut.dout)
...
async def test_a(dut):
    ...
    dut.rst.value = 1
    dut.trigger.value = 0
    dut.din.value = UART_BYTE #set in 16 bit input value
    await ClockCycles(dut.clk, 3) #wait three clock cycles
    assert dut.dout.value.integer==1, "dout is not 1 on reset!"
...
def uart_runner():
    ...
    parameters = {'INPUT_CLOCK_FREQ': 100000000, 'BAUD_RATE': 10000000} #!!!change these to do different versions
