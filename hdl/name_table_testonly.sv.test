...
module name_table ( 
   ...
   logic [10:0] actual_wr_addr;
    xilinx_true_dual_port_read_first_2_clock_ram #(
        ...
        .RAM_DEPTH(2048),
        .INIT_FILE(`FPATH(smb_nametable_sample.mem))) // 2 name tables
    name_table (
    ...
    always_comb begin
      if (is_mirroring_horiz) begin
         if (16'h000 <= read_addr && read_addr < 16'h400) begin
            actual_rd_addr = read_addr[9:0];
         end else if (16'h400 <= read_addr && read_addr < 16'h800) begin
            actual_rd_addr = read_addr[9:0];
         end else if (16'h800 <= read_addr && read_addr < 16'hC00) begin
            actual_rd_addr = 1024 + read_addr[9:0];
         end else begin
            actual_rd_addr = 1024 + read_addr[9:0];
         end


         if (16'h000 <= write_addr && write_addr < 16'h400) begin
            actual_wr_addr = write_addr[9:0];
         end else if (16'h400 <= write_addr && write_addr < 16'h800) begin
            actual_wr_addr = write_addr[9:0];
         end else if (16'h800 <= write_addr && write_addr < 16'hC00) begin
            actual_wr_addr = 1024 + write_addr[9:0];
         end else begin
            actual_wr_addr = 1024 + write_addr[9:0];
         end
      end else begin
